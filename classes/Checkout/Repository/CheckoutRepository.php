...
class CheckoutRepository {
    ...
    public function placeOrder(int $customer_id, array $orderData, array $cartItems, ?array $paymentProof): bool {
        ...
        try {
            // 1. Insert order — use delivery_address (correct column name)
            $stmt1 = $this->conn->prepare("
                INSERT INTO tbl_orders
                    (customer_id, total_price, order_status, payment_method,
                     delivery_option, order_reference_no, delivery_address)
                VALUES (?, ?, 'PENDING', ?, ?, ?, ?)
            ");
            $stmt1->bind_param("idssss",
                $customer_id,
                $orderData['total_price'],
                $orderData['payment_method'],
                $orderData['delivery_option'],
                $orderData['reference_no'],
                $orderData['delivery_address']   // fixed: was shipping_address
            );
            $stmt1->execute();
            $order_id = $this->conn->insert_id;

            // 2. Insert payment — use PENDING (uppercase, matches your ENUM)
            ...
            // 3. Insert payment proof upload if GCash
            if ($paymentProof) {
            ...
            // 4. Move cart items to order — update source_type and attach order_id
            ...
            // 5. Also update any linked printing order items with the order_id
